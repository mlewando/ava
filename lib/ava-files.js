...
function handlePaths(files, excludePatterns, extensions, globOptions) {
	...
	return files
		.map(file => {
			...
			if (fs.statSync(file).isDirectory()) {
				...
				return handlePaths([pattern], excludePatterns, extensions, globOptions);
			}
		...
		.then(flatten)
		.filter(file => file && extensions.indexOf(path.extname(file)) >= 0)
		.filter(file => {
...
class AvaFiles {
	...
	findTestFiles() {
		return handlePaths(this.files, this.excludePatterns, this.extensions, {
			cwd: this.cwd,
	...
	findTestHelpers() {
		return handlePaths(defaultHelperPatterns(), ['!**/node_modules/**'], this.extensions, {
			cwd: this.cwd,
